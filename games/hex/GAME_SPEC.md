...
- 独立 Bridge V2 Setup/Play/Replay Surface 只消费投影视图，发送 `SELECT_STARTER`、`PLACE_STONE` 或平台确认后的 `RESIGN` intent；平台拥有 ready、关闭/离开、连接和 replay 生命周期。artifact 与画面说明见 [Surface README](../../game-surfaces/hex/README.md)。
- Surface 只显示服务器给出的 `winningPath`，不得导入 Core、重跑 BFS、推断连接胜者或构造 Outcome；纯表现层升级不改变 `gameVersion` 或 Replay Format。
