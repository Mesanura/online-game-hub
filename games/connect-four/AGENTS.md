...
- 当前 Action 只表达 `DROP_DISC(column)` 或 strict `RESIGN` 意图；不得包含 actor、State、落点 row、随机结果或 Outcome。
- `RESIGN` 在活跃比赛中不受回合限制；`connect-four@1.0.0` definition 必须保持独立冻结且继续拒绝该 Action。
