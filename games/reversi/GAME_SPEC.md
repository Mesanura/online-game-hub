...
- 独立 `reversi@surfaceVersion 1.0.4` Bridge V2 artifact 承载 Setup、Play 与 Replay，并同时解析历史 `1.0.0` 和 current `1.1.0` projected View。Surface 使用暖木 Clay 棋盘和 teal 合法落点，显式固定八行八列等分轨道，状态更新不得改变棋格或棋盘尺寸。Surface 只启用服务器给出的 `legalMoves`，提交最小 `PLACE_DISC` intent，不自行扫描夹线或计算 Outcome。
- 历史 `1.0.0` 已登记 V6 Setup，复用相同的 `null` Config 与参与者顺序契约；存量 V5 房间仍保持创建时代际。同一 exact-version Surface 渲染 Setup/Play/Replay。Surface 视觉升级不改变 `gameVersion` 或 Replay Format。
