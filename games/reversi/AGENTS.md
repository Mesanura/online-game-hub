...
- 当前 Action 只表达 strict `PLACE_DISC(cell) | RESIGN` 意图；不得包含 actor、State、翻转列表、Outcome、revision、PASS 或随机结果。
- `RESIGN` 在活跃比赛中不受回合限制；`reversi@1.0.0` definition 必须保持独立冻结且继续拒绝该 Action。
...
- 游戏没有隐藏信息，但所有出站内容仍由 `projectView` 生成；Surface 与保留的 Client Module 均不得导入 Core、扫描夹线或自行判断跳过与 Outcome。
