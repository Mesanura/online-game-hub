...
Online Game Hub 是一个面向双人在线棋类游戏的 TypeScript monorepo。平台采用服务端权威模型：浏览器只提交操作意图，Game Server 负责验证规则、推进状态和记录回放。
...
- 已完成比赛的 PostgreSQL 持久化、私有历史记录和可验证的 canonical replay；
- 显式注册的 Game Plugin，可通过 `pnpm create-game --game-id <id>` 创建开发骨架。

当前版本聚焦双人实时对局。账号体系、匹配大厅、观战、公开回放、多实例房间协调等能力尚未实现。
