...
| [像素炸弹人](../games/bomberman/GAME_SPEC.md)      | 2–4 人、固定竞技场与对称随机砖块、60 Hz 整数碰撞/连锁爆炸、一局三命、受击无敌与加成随机散落、半格转弯辅助；居中对称人物、定向火焰及持续音效，Phaser/双指摇杆，历史规则独立冻结，record-only |
...
| 单元     | 状态   | 范围与退出条件                                                         |
| -------- | ------ | ---------------------------------------------------------------------- |
| 卡片展示 | 已完成 | 首页与目录复用卡片，类型与人数准确，顺序、链接与真实浏览器导航通过验证 |
| 目录筛选 | 已完成 | 标题/简介搜索、类型/人数组合筛选、URL 恢复、空结果与可访问操作         |
| 历史体验 | 已完成 | 最近 50 条内按游戏/状态筛选，加载失败重试、请求清理与权限/回放回归     |
