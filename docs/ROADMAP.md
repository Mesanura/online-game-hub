...
> 实施状态：已完成；回合制与 realtime runtime 共用 Setup coordinator，井字棋和 Pong 的 production registration 已切换为 V6，两类 RoomStore 及 PostgreSQL adapter 均持久化 Setup 状态与上一局最终设置。
...
> 实施状态：已完成；井字棋 React/Vite 与 Pong TypeScript/Phaser 均以独立 Setup/Play/Replay artifact 接入 production registration，并通过 contract、integration、真实 PostgreSQL 与双浏览器 E2E。
...
> 实施状态：进行中；全部当前和受支持历史 `gameVersion` 均已有 exact Bridge V2 Surface 映射，Web live room/replay 已停止加载 legacy Client Module，终局 HUD、暖木棋盘以及四子棋/黑白棋/Pong 跨缩放几何稳定性修复已完成；只剩存量 V5 room runtime 在排空后退役。
