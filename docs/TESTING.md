...
| [羽毛球](../games/badminton/GAME_SPEC.md)          | 各比分 Config、半场/跳跃、输入有效期、三类击球/冷却/重复触球、球网高速拦截/落地/出界、领先两分与封顶；手动/空中发球、非发球方拒绝、长按不连发、起手时序/前脚对齐、阻力和历史自动发球；左右网前挑球脱困与退守接回、多种起跳/挥拍时机、低弧线过网、扣杀上下行/身前/高度/角度降级、方向落点和触球后回收                 |
...
像素炸弹人额外覆盖 2／3／4 人、镜像砖块/隐藏道具与安全出生、半格转弯预算与提前转向阻挡、玩家与炸弹碰撞、道具上限、三命及无敌到期边界、受击继续移动/放弹、方向火焰分别过期、阻挡快照与连锁爆炸、同 tick 全灭/自伤/拾取争抢、180 秒超时和永久投降。每个人数保留当前三命与历史抢三胜 golden，较大测试地图与额外席位验证内部集合及容量扩展；公开投影不得包含隐藏/待显现道具、RNG 或控制租期。规则见 [GAME_SPEC](../games/bomberman/GAME_SPEC.md)。
...
像素炸弹人使用真实 2／3／4 客户端验证人数设置/准备、权限与 schema 拒绝、同 tick 有序 MOVE/PLACE_BOMB、容量不足不补放、duplicate/stale/错轮不入 record、投影隐私、剩余生命/无敌与稳定席位重连、三次受击后的单场终局、投降及完整设置重开。临时 PostgreSQL 验证新旧版本归档、跨连接 exact record 重建、账户隔离及胜/负/平战绩。
...
像素炸弹人的独立浏览器契约验证完整 13×11 像素地图、最近邻缩放、四人生命/倒计时/结果摘要及历史胜场；Canvas 验证方向火焰、人物居中与无敌闪烁/reduced-motion，原生 Web Audio 验证泡泡去重、持续爆炸/连锁续接、到期及失焦/静音/重连/销毁停止。键盘覆盖最后按下方向、Space 边沿和 150ms 续租；Chromium 真实多指覆盖摇杆死区/主轴、与放弹同时按下、滑入按钮不触发、释放/取消/失焦/断线/阶段/场次/销毁清理，受击不清控制、零命才清理。新旧规则的横竖屏检查自然几何、实际点击命中、焦点框不覆盖状态/能力栏、44px 目标和平台全屏按钮避让。四浏览器账户旅程完成三命终局、刷新恢复、下一场与私有历史，record-only 没有播放入口。
