...
等待页以复制按钮提供不含身份凭据的邀请 URL，不长期暴露原始 URL；复制需要 loading/success/failure 与可手动选择的后备。实际对局使用占满可用空间的游戏舞台，平台 HUD 是默认收起且不参与舞台尺寸计算的覆盖式抽屉；左上角只保留抽屉开关，右下角独立提供全屏开关，连接详情只在抽屉与辅助技术状态中出现。舞台全屏失败时降级为 `100dvh` focus mode。Round 完成后，右侧覆盖式终局 HUD 立即显示通用完成状态，再用 Bridge V2 Surface 提供的纯文本摘要替换结果文案，并提供逐玩家“重新对局”确认和“调整设置”；它不探测游戏 View 中的比分、棋子、阵营、当前回合、排名或 Outcome。大型棋盘或 canvas 的滚动、缩放与响应式布局由各 Surface 负责，并在移动端保留至少 44px 的有效操作目标。
...
中国跳棋 current `1.1.0` 支持 2–6 名玩家，每位玩家拥有六枚棋子和六角星角部的唯一营地。棋盘固定为 73 个轴坐标棋位：中央边长 4 的正六边形与六个边长 3 的正三角营地；旧 `1.0.0` 按原编号和拓扑保留历史回放。玩家可相邻移动或连续跳跃，客户端只提交起点与终点。等待页允许房主选择本轮人数、玩家选择/取消营地，并在全部在线、唯一 assignment、先手和 ready 条件满足后开局；对局按完成、阻塞和投降原因生成严格排名。该游戏复用稳定席位、重连、多人 lifecycle、Replay Format V1、PostgreSQL archive/history 和通用投降 HUD，不加入 AI、计时、悔棋或交换规则。

### 4.6 额外实时游戏：火柴人羽毛球

当前 `badminton@1.1.0` 提供原创火柴人双人联机羽毛球。双方在各自半场移动、跳跃，选择高远球、吊球或跳跃扣杀；服务器以 60 Hz 整数模拟裁定击球、触网、落地、出界、每球得分与胜负。房主在 Setup 中选择 7/11/21 分和首发方；领先 2 分获胜，分别以 11/15/30 分封顶，每球胜者按 S 手动发球，等待不限时，可在己方后三分之二带球移动和空中发球，前进上限的前脚与场内竖线对齐。低位自动下手挑球，球路使用横纵阻力；独立 Surface 提供中线站位、透视双立柱球网、粒子拖尾与可关闭的挥拍/击球音效。历史 `1.0.0` 自动发球及旧轨迹冻结保留。详细物理、判定与控制语义由 [游戏规格](../games/badminton/GAME_SPEC.md) 定义。

游戏从统一目录与 `/games/badminton` 进入，复用邀请、stable slots、双方 ready、60 秒重连、平台投降、同房间多轮及账户战绩。键盘和手机多指触控均可操作；失焦、触控取消与断线释放输入，服务器还限制持续输入的有效时长。独立 Setup/Play Surface 使用 Phaser 绘制，客户端只提交操作意图并显示公开快照，不运行本地权威物理。

本次是 M8 完成后的单款内容扩展，不改变 M8 当时仅验证 Pong 的范围，也不新增平台能力。首版 replay 显式为 `record-only`，内部保留可确定性验证的 canonical record，账户历史显示战绩但无玩家回放。暂不包含 AI、单机对手、双打、匹配、观战、技能成长或公开回放。
