...
- 对局页不复用等待页的邀请控件或额外连接详情；平台 HUD 是默认收起、不参与舞台尺寸计算的覆盖式抽屉，左上角为抽屉开关、右下角为独立全屏开关，连接详情不再作为常驻可见徽标。HUD 从 `room.roomCode` 显示房间码，仅在 exact deployment 的 `platformControls` 声明 `RESIGN` 时显示二次确认，并依据 server lifecycle 的 `isOwner` 选择 `closeRoom()` 或 `leaveRoom()`。Round completed 后右侧非模态终局 HUD 使用与最近 `host.state.sequence` 匹配的 Bridge V2 纯文本摘要；重新对局完全读取 V6 lifecycle readiness，已确认玩家可再次点击取消。投降确认后，Host 只向 Surface 发送无 payload 的受限命令，由 Surface 生成游戏自有 Action/Input intent；关闭/离开在 active Round 仍独立确认，UI 不自行裁定 resignation、关闭或 abandoned 结果。
