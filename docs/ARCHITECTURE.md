...
| `game-server-ticket`  | Web issuer 与 Game Server verifier 共用的短期 HMAC-SHA256 ticket authority，实现 Protocol V2 ticket claims       | 浏览器 API、session cookie、房间/游戏规则、testing authority |
...
- command outcome cache 以 `PlayerSessionId + commandId` 为 key，Protocol V2 保留整个 live room lifetime，包括同房间后续轮次。旧轮重复命令返回带旧 `roundNumber` 的原结果，但不会再次进入 Core 或覆盖新轮 snapshot；后续长房间可在不小于重试窗口的前提下加入有界淘汰。
