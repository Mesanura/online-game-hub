...
- 对局页不复用等待页的邀请控件或额外连接详情；平台 HUD 是默认收起、不参与舞台尺寸计算的覆盖式抽屉，最小浮动工具条只保留打开 HUD、连接状态和全屏。HUD 从 `room.roomCode` 显示房间码，仅在 exact deployment 的 `platformControls` 声明 `RESIGN` 时显示二次确认，并依据 server lifecycle 的 `isOwner` 选择 `closeRoom()` 或 `leaveRoom()`。投降确认后，Host 只向 Surface 发送无 payload 的受限命令，由 Surface 生成游戏自有 Action/Input intent；关闭/离开在 active Round 仍独立确认，UI 不自行裁定 resignation、关闭或 abandoned 结果。
