...
- 当前五个游戏都是 `minPlayers = maxPlayers = 2`。stable slot 与 `PlayerSessionId` 映射在 live room 内保持不变；每轮有独立有序 `playerOrder`，由房主的 OWNER/NON_OWNER/RANDOM 选择决定。RANDOM 通过本轮 seed 决定顺序但不推进交给 Core 的 RNG cursor；Core 初始化与 Replay header 必须使用完全相同的最终顺序。
