...
本文定义固定 tick simulation、输入交付、实时协议和客户端职责。当前消费者为 Pong、火柴人羽毛球、坦克迷战、气垫球、像素炸弹人与像素忍战；游戏规则和版本见 [游戏索引](../games/README.md)。共享房间协议见 [NETWORK_PROTOCOL.md](./NETWORK_PROTOCOL.md)，记录格式见 [REPLAY_DESIGN.md](./REPLAY_DESIGN.md)。
...
- `game-surfaces/<id>`：独立画面；Pong/羽毛球/气垫球/像素炸弹人/像素忍战使用 Phaser，坦克迷战使用 SVG，均只依赖 Bridge 和自身渲染栈。
