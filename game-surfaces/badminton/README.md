...
Setup 的三种比分选项分别显示封顶分数；`1.5.0` 规则页额外提供标准、加速、疾速三个球速挡位。已确认摘要与轻量球场示意解释领先两分、封顶获胜与首发站位。发球提示遵循 [规则版本说明](../../games/badminton/GAME_SPEC.md#setup投影与版本)。投影更新保留有效键盘焦点和滚动位置；提交中禁止重复操作，拒绝后继续显示服务器已确认选择，并区分权限、过期与连接失败。断线和新局清理等待状态。
