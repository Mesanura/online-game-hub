...
A/D 或左右方向键移动，W/上方向键/空格起跳，J 高远球、K 扣杀、L 吊球；触屏支持多指同时移动、起跳和挥拍。blur、visibilitychange、pointercancel、lostpointercapture、断线与 dispose 清除操作；持续按住时每 150ms 刷新输入。投降期间停止其他输入，避免同 tick 覆盖投降。

只有同一回合、同一阶段且连续的公开快照进行显示插值；重新发球、重连和终局立即收敛。reduced-motion 关闭拖尾、摆腿和位置插值。终局摘要由本 Surface 以中文纯文本提交给平台 HUD。
