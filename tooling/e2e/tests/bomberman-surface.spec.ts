...
async function publicView(): Promise<PublicView> {
  return JSON.parse(
    await readFile(
      new URL(
        "../../../game-surfaces/bomberman/tests/fixtures/play.json",
        import.meta.url,
      ),
      "utf8",
    ),
...
test("bomberman renders the complete pixel arena, four scores and accessible controls across viewports", async ({
  browser,
}, info) => {
  ...
  try {
    const { surface, push } = await openPlaySurface(page, "bomberman", "1.0.0");
    await push(await publicView(), { tick: 180 });
    await expect(surface.locator(".score-card")).toHaveCount(4);
    await expect(surface.locator("#match-timer")).toHaveText("03:00");
...
test("bomberman keyboard keeps the last held direction, renews leases and never repeats bombs", async ({
  page,
}) => {
...
for (const viewport of [
  ...
  test(`bomberman real multitouch joystick and bomb remain independent at ${viewport.width}`, async ({
    browser,
  }) => {
...
test("bomberman projected preparation, intermediate draw and terminal summary use exact Bridge metadata", async ({
  ...
  view.outcome = {
    ...
    winnerSlotId: "p0",
    reason: "SCORE",
    scores: view.players.map(({ slotId, score }) => ({ slotId, score })),
  };
