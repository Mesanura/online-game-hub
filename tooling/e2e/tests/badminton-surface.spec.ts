...
for (const version of ["1.0.0", "1.1.0", "1.2.0", "1.3.0", "1.4.0"]) {
  test(`badminton touch layout and legacy serve visibility ${version}`, async ({
    ...
    try {
      ...
      if (["1.2.0", "1.3.0", "1.4.0"].includes(version))
      ...
      if (version === "1.3.0" || version === "1.4.0") {
        ...
        await expect(surface.locator("#control-help")).toContainText(
          version === "1.4.0" ? "上前接球的时间" : "低弧线",
        );
        await expect(surface.getByTestId("tactics-help")).toContainText(
          version === "1.4.0" ? "吊前场" : "挡网前",
        );
...
for (const version of ["1.3.0", "1.4.0"]) {
  test(`badminton ${version} keeps smash placement available to keyboard and real multitouch`, async ({
    browser,
  }) => {
