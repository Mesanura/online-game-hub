...
test("home and catalog show the same twelve games with accurate types and player counts", async ({
  page,
}) => {
  ...
  for (const path of ["/", "/games"]) {
    ...
    await expect(metadata.getByText("2–4 人", { exact: true })).toHaveCount(2);
